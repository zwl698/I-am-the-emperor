...
export class CampaignScene extends Phaser.Scene {
  ...
  preload() {
    this.load.svg('campaign-map', '/assets/map/campaign-map.svg', { width: 1600, height: 900 });
  ...
  private drawCities(width: number, height: number, snapshot: GameSnapshot) {
    ...
    for (const city of snapshot.cities) {
      ...
      const halo = this.add.circle(point.x, point.y, compact ? 21 : 29, tint, 0.2);
      halo.setStrokeStyle(2, 0xffe6a8, 0.42);
      ...
      marker.setDisplaySize(compact ? 33 : 48, compact ? 33 : 48);
      marker.setTint(tint);
      marker.setInteractive({ useHandCursor: true });
      marker.on('pointerdown', () => this.onCitySelected(city.id));

      if (city.ownerId !== 'neutral') {
        const banner = this.add.image(point.x + 22, point.y - 22, 'army-banner');
        banner.setDisplaySize(compact ? 18 : 28, compact ? 18 : 28);
        banner.setTint(tint);
      }
      ...
      const label = this.add.text(point.x, point.y + 25, labelText(city, ruler), {
        ...
        color: '#fff1c7',
        backgroundColor: 'rgba(29, 18, 13, 0.72)',
        padding: { x: compact ? 4 : 7, y: compact ? 2 : 4 },
      });
